...
| UI-DETAIL-004 | 预算面板 | 查看面板 | 显示「预算明细」面板，默认折叠 | P0 |
| UI-DETAIL-005 | 预算分类 | 展开预算面板 | 显示交通、住宿、餐饮等分类，每项有小计 | P0 |
| UI-DETAIL-006 | 供应商面板 | 查看面板 | 显示「推荐供应商」面板，默认折叠 | P0 |
| UI-DETAIL-007 | 供应商卡片 | 展开供应商面板 | 显示供应商名称、评分、标签、联系方式 | P0 |
| UI-DETAIL-008 | 联系按钮 | 查看按钮 | 显示「拨打电话」和「复制微信」按钮 | P0 |
...
| FUNC-DETAIL-003 | 预算展开/折叠 | 在详情页 | 1. 点击预算面板标题 | 面板展开/折叠 | P1 |
| FUNC-DETAIL-004 | 供应商展开/折叠 | 在详情页 | 1. 点击供应商面板标题 | 面板展开/折叠 | P1 |
| FUNC-DETAIL-005 | 拨打电话 | 在详情页 | 1. 点击「拨打电话」 | 调用微信拨号 API，弹出拨号界面 | P0 |
| FUNC-DETAIL-006 | 复制微信号 | 在详情页 | 1. 点击「复制微信」 | 微信号复制到剪贴板，显示成功提示 | P0 |
...
| INT-API-002 | 生成方案 | POST /plans/generate | 1. 传入需求数据<br>2. 等待响应 | 返回 3 个方案数据，包含完整行程、预算、供应商 | P0 |
...
| BOUND-DATA-004 | 供应商超多 | 20+ 供应商 | 正常显示，可滚动 | P2 |
